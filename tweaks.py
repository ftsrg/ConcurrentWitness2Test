...
def reach_error(ast):
    for node in ast.ext:
        if isinstance(node, FuncDef) and node.decl.name == "reach_error":
            ...
            extern_decl = Decl(name=func_name, quals=[], storage=["extern"], funcspec=[],
                               type=node.decl.type, init=None, bitsize=None)
            ast.ext.remove(node)
...
def fix_inline(ast):
    inline_defs = [node for node in ast.ext if
                   isinstance(node, FuncDef) and "inline" in node.decl.funcspec and "static" not in node.decl.storage]
...
def fix_struct_def(ast):
    struct_decls = set()
    for node in ast.ext:
        if isinstance(node, Decl) and isinstance(node.type, TypeDecl) and isinstance(node.type.type, Struct):
